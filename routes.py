...
def line_segmentation(image):
    ...
    image = cv2.resize(
        image,
        (
            int(og_image.shape[1] * LINE_SEGMENTATION_SCALE_FACTOR),
            int(og_image.shape[0] * LINE_SEGMENTATION_SCALE_FACTOR),
        ),
        interpolation=cv2.INTER_AREA,
    )
    brightness_array = np.mean(image, axis=1)
    threshold = (
        np.median(brightness_array) * 0.2 + np.average(brightness_array) * 0.8
    )
    binarized_array = np.where(brightness_array >= threshold, 1, 0).tolist()
    changes_indices = []
    for i in range(1, len(binarized_array)):
        # tried doing a smart numpy thing here and it didn't work
        # for some reason so uhhhh dumb solution ig
        if binarized_array[i] != binarized_array[i - 1]:
            changes_indices.append(i)

    spacings = np.array(
        [
            changes_indices[i] - (0 if i == 0 else changes_indices[i - 1])
            for i in range(0, len(changes_indices))
        ]
    )
    new_changes_indices = []
...
def demo():
    ...
    if request.method == "POST":
        ...
        if request.form.get("multiple_lines") == "true":
            ...
            # threshold = np.median(img_arr) * 0.2 + np.average(img_arr) * 0.8
            # contrast_applied_array = (
            #     np.exp((img_arr - threshold) * -1 * (1 / 255)) + 1
            # ) ** -1
            # image = Image.fromarray(
            #     (contrast_applied_array * 255).astype(np.uint8)
            # )
